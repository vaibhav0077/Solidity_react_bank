...
const WithDrawEther = ({ closeForm, remark, setRemark, isLoading, setIsLoading }) => {
    ...
    const handleSubmit = async (event) => {
        ...
        if (window.ethereum) {
            ...
            try {
                // WITHDRAW AMOUNT
                const sendAmount = await contract.bankToWalletTransfer(ethers.utils.parseEther((event.target.amountEth.value).toString()))
                console.log(sendAmount);
            }
            catch (error) {
                alert(`Amount ${amount} is not Sufficient to withdraw`)
    ...
    return (
        <div className="send-container">
            <form onSubmit={handleSubmit} className="popup-form-send ">
                ...
                <button className="main-connect-button">WithDraw Ether</button>
            </form>
        </div>
    )
}
